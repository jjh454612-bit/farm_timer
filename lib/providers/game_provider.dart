...
class StudyCategory {
  ...
  const StudyCategory({required this.id, required this.name, required this.color});

  Map<String, dynamic> toJson() => {'id': id, 'name': name, 'color': color.toARGB32()};
...
class StudySession {
  ...
  const StudySession({required this.startTime, required this.seconds, required this.categoryId});
...
class GameProvider extends ChangeNotifier {
  ...
  static const List<Color> palette = [
    Color(0xFFF5E6C8), Color(0xFFF5F5F5), Color(0xFFE0E0E0),
    Color(0xFFFFCDD2), Color(0xFFF8BBD9), Color(0xFFFFCCBC),
    Color(0xFFFFE0B2), Color(0xFFFFF9C4), Color(0xFFC8E6C9),
    Color(0xFFB2DFDB), Color(0xFFB3E5FC), Color(0xFFBBDEFB),
    Color(0xFFB2EBF2), Color(0xFFD1C4E9), Color(0xFFE1BEE7),
    Color(0xFFD7CCC8),
  ];
  ...
  // 인구: house 2명, modernhouse 4명, landmark2 300명
  ...
  bool get isTutorialDone => _tutorialStep == -1;
  ...
  bool isMissionComplete(int index) =>
      index < _dailyMissions.length && _todayStudyMinutes >= _dailyMissions[index];

  int missionReward(int minutes) => (minutes ~/ 20) * 100;

  List<StudySession> sessionsForDate(DateTime date) => _sessions.where((s) =>
  ...
  Future<void> load() async {
    ...
    _money              = prefs.getDouble('money') ?? 10000000;
    ...
    if (gridJson != null) _groundGrid = List<String>.from(jsonDecode(gridJson)).toSet();
    ...
    if (rotatedJson != null) {
      _rotatedTiles = List<dynamic>.from(jsonDecode(rotatedJson)).map((e) => e as int).toSet();
    }
    ...
    if (missionsJson != null) _dailyMissions = List<int>.from(jsonDecode(missionsJson));

    final claimedJson = prefs.getString('claimedMissions');
    if (claimedJson != null) _claimedMissions = List<int>.from(jsonDecode(claimedJson)).toSet();

    final categoriesJson = prefs.getString('categories');
    if (categoriesJson != null) {
      final list = List<dynamic>.from(jsonDecode(categoriesJson));
      _categories = list.map((e) => StudyCategory.fromJson(Map<String, dynamic>.from(e))).toList();
    }
    ...
    _tutorialStep = prefs.getInt('tutorialStep') ?? 0;
  ...
  Future<void> _save() async {
    ...
    await prefs.setInt('tutorialStep', _tutorialStep);
    await _saveMissions(null);
    await _saveCategories(null);
    await _saveSessions(null);
  }

  Future<void> _saveMissions(SharedPreferences? p) async {
  ...
  Future<void> _saveCategories(SharedPreferences? p) async {
    final prefs = p ?? await SharedPreferences.getInstance();
  ...
  Future<void> _saveSessions(SharedPreferences? p) async {
    final prefs = p ?? await SharedPreferences.getInstance();
  ...
  void setCharacter(String character) { _character = character; _save(); notifyListeners(); }

  void addMoney(double amount) { _money += amount; _save(); notifyListeners(); }
  void spendMoney(double amount) { _money -= amount; _save(); notifyListeners(); }
  ...
  void placeBuilding(int tileIndex, String buildingId, bool isRotated) {
    ...
    if (isRotated) { _rotatedTiles.add(tileIndex); } else { _rotatedTiles.remove(tileIndex); }
    if (!unlimitedBuildings.contains(buildingId)) {
      final current = _ownedBuildings[buildingId] ?? 0;
      if (current <= 1) { _ownedBuildings.remove(buildingId); }
      else { _ownedBuildings[buildingId] = current - 1; }
    }
    _save();
    notifyListeners();
  }
  ...
  void buyLandVoucher() {
    ...
    if (!canAfford(price)) return;
    _money -= price; _landVouchers++; _totalLandPurchased++;
    _save(); notifyListeners();
  }

  void useLandVoucher(int gx, int gy) {
    if (_landVouchers <= 0) return;
    _landVouchers--; _groundGrid.add('$gx,$gy');
    _save(); notifyListeners();
  }

  void addStudyMinutes(int minutes) {
    _todayStudyMinutes += minutes; _saveMissions(null); notifyListeners();
  }

  void addSession(int seconds, DateTime startTime) {
    if (seconds < 60) return;
    _sessions.add(StudySession(startTime: startTime, seconds: seconds, categoryId: _selectedCategoryId));
    final cutoff = DateTime.now().subtract(const Duration(days: 90));
    _sessions.removeWhere((s) => s.startTime.isBefore(cutoff));
    _saveSessions(null); notifyListeners();
  }
  ...
  bool usePause() {
    ...
    _pauseCount++; _savePauseCount(); notifyListeners(); return true;
  }

  void selectCategory(String id) {
    if (_categories.every((c) => c.id != id)) return;
    _selectedCategoryId = id; _saveCategories(null); notifyListeners();
  }
  ...
  void addCategory(String name, Color color) {
    ...
    _saveCategories(null); notifyListeners();
  }

  void removeCategory(String id) {
    if (_categories.length <= 1) return;
    _categories.removeWhere((c) => c.id == id);
    if (_selectedCategoryId == id) _selectedCategoryId = _categories.first.id;
    _saveCategories(null); notifyListeners();
  }

  void startTutorial() { _tutorialStep = 1; _save(); notifyListeners(); }

  void nextTutorialStep() {
    if (_tutorialStep == -1) return;
    _tutorialStep++;
    if (_tutorialStep > 4) _tutorialStep = -1;
    _save(); notifyListeners();
  }

  void completeTutorial() { _tutorialStep = -1; _save(); notifyListeners(); }

  void giveTutorialHouse() { _ownedBuildings['tthouse'] = 1; _save(); notifyListeners(); }

  void demolishBuilding(int tileIndex) {
    final buildingId = _placedBuildings[tileIndex];
    if (buildingId == null) return;
    _placedBuildings.remove(tileIndex);
    _rotatedTiles.remove(tileIndex);
    // 수량 건물이면 인벤토리에 반환
    if (!unlimitedBuildings.contains(buildingId) && buildingId != 'tthouse') {
      _ownedBuildings[buildingId] = (_ownedBuildings[buildingId] ?? 0) + 1;
    }
    _save();
    notifyListeners();
  }

  void setThemeColor(Color color) { _themeColor = color; _save(); notifyListeners(); }
}